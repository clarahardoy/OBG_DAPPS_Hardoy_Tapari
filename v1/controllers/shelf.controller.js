...
export const ShelfController = {
	...
	createShelf: async (req, res) => {
		try {
			...
			const shelves = await ShelfService.getUserShelves(userId);
			if (shelves.length >= 1) {
				const canCreateMultiple = await ShelfService.canCreateMoreThanOneShelf(
					userId
				);
	...
	updateShelf: async (req, res) => {
		try {
			...
			if (shelf.isDefault && updateData.isDefault === false) {
				return res.status(400).json({
					error: 'No se puede cambiar el estado de la biblioteca principal',
				});
			}
	...
	validateShelfHasSpaceLeft: async (req, res) => {
		...
		res
			.status(200)
			.json({ message: 'Shelf tiene espacio libre', hasSpaceLeft });
	},
	...
	validateShelfBelongsToUser: async (req, res) => {
		...
		res
			.status(200)
			.json({ message: 'Shelf pertenece al usuario', belongsToUser: true });
	},
};
